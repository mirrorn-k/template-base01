...
import * as TypeForm from "./type";
import * as taEndInput from "@/lib/api/endInput/type";
import * as apiEndInput from "@/lib/api/endInput";
...
const Main = () => {
  ...
  const { cfItems } = ContextMap.Contents();
  ...
  const handleOnClick = async () => {
    ...
    // postデータの作成
    const inputs: taEndInput.tInputItem[] = cfItems.map((item) => {
      return {
  ...
  return (
    ...
    >
      <Grid container spacing={2}>
        {cfItems.map((item: TypeForm.tFormItem, index) => (
